...
//10 رندوم 4
// 20 random5
int microphonedigitall = 16;
  int microphonedigitoranalouge = 5;
  int withoutmicrophone = 14;
int microphonedigitallvalue,microphonedigitoranalougestat;
...
analogWriteRange(255);
  ...
  pinMode(A0, INPUT); pinMode(resetbottom, INPUT); pinMode(microphonedigitall, INPUT_PULLUP);  pinMode(microphonedigitoranalouge, INPUT_PULLUP); 

 pinMode(microphonedigitoranalouge, INPUT_PULLUP);// if 0  digitalll     //if 1 analouge
    
      
      pinMode(withoutmicrophone, INPUT_PULLUP);
...
//  
 ...
 SPIFFS.begin();
 if(digitalRead(withoutmicrophone) == 1){
 server.on("/"               ,[](){File f=SPIFFS.open("/index.htm"      ,"r"); Serial.println("---------------");server.streamFile(f,"text/html"      );f.close();});
   ...
   server.on("/"               ,[](){File f=SPIFFS.open("/index1.htm"      ,"r"); Serial.println("---------------");server.streamFile(f,"text/html"      );f.close();});
    server.on("/index.htm"      ,[](){File f=SPIFFS.open("/index1.htm"      ,"r"); Serial.println("---------------");server.streamFile(f,"text/html"      );f.close();});
...
flasherdelaylocal =  400; 
  ...
  pinMode(2, OUTPUT); digitalWrite(2, LOW); 
   if(digitalRead(microphonedigitoranalouge) == 0){
       microphonedigitoranalougestat = 0;
   }else{
       microphonedigitoranalougestat = 1;
   }
